...
export default function SleepTracker() {
  ...
  if (loading) return <LottieLoader text="Loading sleep data..." />;
        ...
        <div className="px-5 pb-6">
          ...
          {tab === 2 && (
            !summary || summary.logCount === 0 ? (
              <div className="flex flex-col items-center justify-center py-16"><MeditationLottie /><p className="text-lg font-bold text-white mb-2">No Summary Yet</p><p className="text-[13px] text-white/40">Log your sleep for a few days to see insights.</p></div>
            ) : (
              <div className="space-y-4 pt-2">
                {/* Date selector */}
...
function MeditationLottie() {
  ...
  useEffect(() => { fetch('/meditation.json').then(r => r.json()).then(setAnim).catch(() => {}); }, []);
  if (!anim) return <div className="h-[150px]" />;
  return <Lottie animationData={anim} loop style={{ height: 150 }} />;
}
